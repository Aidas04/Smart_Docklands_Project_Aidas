...
import java.util.ArrayList; /* Required to import java.util to allow 
    ArrayList/Stack*/
...
public class MyStack implements StackInterface{ /* This class implements the 
    StackInterface class, allowing to provide functions to the methods.*/
    
    private ArrayList<Issue> stack = new ArrayList<>(); /* Here i declare the 
    ArrayList called Stack that uses Issue.java as its Object.*/
    ...
    /* peek() uses the get(0) method to get the top element of the stack 
    if not empty.*/
    
    public Object peek(){
        if(!(stack.isEmpty())){
        return stack.get(0);
        } else{ // If the stack is empty then it will be the message below
